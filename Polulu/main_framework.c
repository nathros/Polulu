...
//IF: intersection is found 90 deg turn
//ELSE: LINE_FOUND_FLAG = 0;
void main_line_intersection()
{
    int i = 0;

    //At EoL reverse n times or untill turn detected
    while((i < LINE_RVS_CNT) || (!pid_turn_detect())) {
        motor_set(MOTOR_LEFT,-18);
        motor_set(MOTOR_RIGHT,-18);
        delay_start(50);
        motors_stop();
    }


    if(pid_turn_detect() == 1) {
        motor_rot_rel(-120);
    }

    else if(pid_turn_detect() == 2) {
        motor_rot_rel(120);
    }
...
void main_line_follow()
{
    ...
    while(1) {
        ...
        pid_run();
        main_line_intersection();   //IF: intersection is found 90 deg turn
                                      //ELSE: LINE_FOUND_FLAG = 0;
    }
}
...
//-Fwd move 220cm----------------------------------------------------*/
            case 1:
                ...
                motor_bias_set(MOTOR_LEFT, 0.5);
                motor_bias_set(MOTOR_RIGHT, 0.4);
                motor_travel(220);
