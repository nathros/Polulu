...
void pid_set_motors(int power_diff)
{
    ...
    if( power_diff < -MAX_SPEED)
        ...
        if(power_diff < 0) {
            left_speed  = (MIN_SPEED + MAX_SPEED +power_diff);
            right_speed = (MIN_SPEED + MAX_SPEED);
        }
...
void pid_control_loop()
{
    ...
    motor_diff = pid_calc(p, &last_p, &i, &d);
    pid_motor_set(motor_diff);
    pid_exit_check();
    }
}
#endif
...
void pid_run()
{
    ...
    if (PID_RUNNING || !PID_FLAG) return;
...
int pid_calc(int prop_current, int *prop_last, int *integral, int *derivative)
{
    ...
    *integral += prop_current;
    int power_difference = (prop_current/Kp + *integral/Ki + *derivative * 2/Kd);
